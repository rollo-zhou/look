...
module.exports = {
  ...
  textBase1: '#009688',
  textBase2: '#d8d2d6',
  apiUserServer: 'http://api.lookbook.nu/v2/user/',
  apiLookServer: 'http://api.lookbook.nu/v2/look/',
  apiServer: 'http://api.lookbook.nu/v2/',
  apiServerHeaders: {
  ...
  queryRromServer(apiUrl, callBack, parameter) {
    ...
    Storage.getItem('cookie').then((cookie) => {
        ...
        Storage.getItem('access_token').then((access_token) => {
            _this.apiServerHeaders["Authorization"] = access_token || "";
            fetch(
                apiUrl, {
